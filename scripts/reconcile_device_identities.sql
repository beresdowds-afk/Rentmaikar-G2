...
BEGIN
  ...
  IF NOT FOUND THEN
    SELECT * INTO s FROM public.iot_sim_cards WHERE false;
  END IF;

  IF d.vehicle_id IS NOT NULL THEN
    SELECT * INTO v FROM public.vehicles WHERE id = d.vehicle_id;
    IF NOT FOUND THEN SELECT * INTO v FROM public.vehicles WHERE false; END IF;

    SELECT * INTO c FROM public.vehicle_mqtt_credentials 
     WHERE vehicle_id::text = d.vehicle_id::text AND (iot_device_id = d.id OR iot_device_id IS NULL) 
     ORDER BY is_active DESC, updated_at DESC LIMIT 1;
    IF NOT FOUND THEN SELECT * INTO c FROM public.vehicle_mqtt_credentials WHERE false; END IF;

    SELECT * INTO r FROM public.rentals 
     WHERE vehicle_id = d.vehicle_id AND status IN ('active','ongoing','in_progress') 
     ORDER BY start_date DESC LIMIT 1;
    IF NOT FOUND THEN SELECT * INTO r FROM public.rentals WHERE false; END IF;
  ELSE
    SELECT * INTO v FROM public.vehicles WHERE false;
    SELECT * INTO c FROM public.vehicle_mqtt_credentials WHERE false;
    SELECT * INTO r FROM public.rentals WHERE false;
  END IF;
  ...
  ELSIF d.vehicle_id IS NOT NULL AND EXISTS (
      SELECT 1 FROM public.vehicle_telemetry_state ts WHERE ts.vehicle_id::text = d.vehicle_id::text
    ) THEN
    tracking_linked := true;
...
GRANT EXECUTE ON FUNCTION public.sync_device_identity(uuid) TO authenticated, service_role, postgres;
...
GRANT EXECUTE ON FUNCTION public.rebuild_all_device_identities() TO authenticated, service_role, postgres;
